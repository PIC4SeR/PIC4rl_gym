...
class Pic4rlEnv_Lidar(py_environment.PyEnvironment, Node):
    """
    """
    def __init__(self, env_name="Generic_Pic4rlEnv_Lidar"):
        """
        """
    ...
    def define_env(self, params):
        ...
        for i in range(params.lidar_points):
            state = state + [[0., params.lidar_distance]]
    ...
    def _step(self, action=np.array([]), reset_step=False):
        ...
        self.state = self.get_observation(lidar_measurements, goal_info)

        if reset_step:
            reward = 0
        else:
            reward = self.get_reward(goal_info, event)
    ...
    def get_reward(self, goal_info, event):
        """
        """
