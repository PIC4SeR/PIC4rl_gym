...
class Pic4rlEnv_Lidar_PF(py_environment.PyEnvironment, Node):
    """
    """
    def __init__(self, env_name="Generic_Pic4rlEnv_Lidar_PF"):
        """
        """
        ...
        self.get_entity_client = GetEntityClient()
    ...
    def define_env(self, params):
        """
        """
        action = np.array([params.min_ang_vel, params.max_ang_vel])

        self._action_spec = array_spec.BoundedArraySpec(
            shape=(1,), 
            dtype=np.float32, 
            minimum=np.array(action[0], dtype=np.float32),
            maximum=np.array(action[1], dtype=np.float32), 
            name='action'
        )
        ...
        [params.min_ang_vel, params.max_ang_vel]
        ]
    ...
    def _step(self, action=np.array([]), reset_step=False):
        ...
        self.state = self.get_observation(action, lidar_measurements, goal_info)
        ...
        self.update(action, lidar_measurements, goal_info, robot_pose)
    ...
    def get_reward(self, action, goal_info, event):
        ...
        smooth_reward = -math.fabs(self.previous_action-action)
    ...
    def get_observation(self, lidar_measurements, goal_info):
        """
        """
    ...
    def update(self, action, lidar_measurements, goal_info, robot_pose):
        """
        """
