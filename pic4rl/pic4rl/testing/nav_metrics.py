...
def tf_compose(robot_pose, goal_pose):
    ...
    goal_pose = np.concatenate((goal_pose, [0.0, 1.0]), axis=0)
...
class Navigation_Metrics(Node):
    def __init__(self, logdir):
        ...
        self.params = {
            "robot_name": self.get_parameter("robot_name")
            .get_parameter_value()
            .string_value,
            "robot_poses": self.get_parameter("robot_poses")
            .get_parameter_value()
            .bool_value,
            "robot_velocities": self.get_parameter("robot_velocities")
            .get_parameter_value()
            .bool_value,
            "path_distance": self.get_parameter("path_distance")
            .get_parameter_value()
            .bool_value,
            "distance_path_ratio": self.get_parameter("distance_path_ratio")
            .get_parameter_value()
            .bool_value,
            "clearance_time": self.get_parameter("clearance_time")
            .get_parameter_value()
            .bool_value,
            "mean_velocities": self.get_parameter("mean_velocities")
            .get_parameter_value()
            .bool_value,
            "max_min_accelerations": self.get_parameter("max_min_accelerations")
            .get_parameter_value()
            .bool_value,
            "cumulative_heading_average": self.get_parameter(
    ...
    def init_get_entity_client(self):
        ...
        self.robot_pose_client = self.create_client(
            GetEntityState, "/test/get_entity_state"
        )
    ...
    def acquire_data(self):
        ...
        robot_pose = [
            ...
            quat_to_euler(data.state.pose.orientation.z, data.state.pose.orientation.w),
        ]

        velocity = [
            np.hypot(data.state.twist.linear.x, data.state.twist.linear.y),
    ...
    def open_logdir(self, logdir):
        """ """
        logging.basicConfig(
            filename=os.path.join(logdir, "metrics_results.log"), level=logging.INFO
        )
    ...
    def calc_metrics(
        ...
        if self.params["obstacle_clearance"]:
            self.obstacle_clearance()
        if self.params["row_crop_path_comparison"]:
            self.row_crop_path_comparison(
                path_coordinate, true_path, plants_row1, plants_row2
            )
    ...
    def save_metrics_results(self, episode):
        """ """
        episode_results = {
            f"episode{episode+1}": dict(
                (metric["name"], metric["value"]) for metric in self.metrics_results
            )
        }
    ...
    def distance_path_ratio(self, start_pose, goal_pose, event="Goal"):
        """ """
        distance = np.hypot(start_pose[0] - goal_pose[0], start_pose[1] - goal_pose[1])
        ...
        self.metrics_results.append(self.create_metric("Distance", float(distance)))
        self.metrics_results.append(
            self.create_metric("Path_distance", float(path_distance))
        )
        self.metrics_results.append(
            self.create_metric("Distance_path_ratio", float(distance / path_distance))
        )
    ...
    def clearance_time(self, event="Goal"):
        ...
        self.metrics_results.append(
            self.create_metric(
                "Clearance_time", float(self.previous_time - self.start_time)
            )
        )
    ...
    def mean_velocities(self, event="Goal"):
        ...
        self.metrics_results.append(
            self.create_metric(
                "Mean_velocities", [float(np.mean(v)), float(np.mean(om))]
            )
        )
        self.metrics_results.append(
            self.create_metric("Std_velocities", [float(np.std(v)), float(np.std(om))])
        )
    ...
    def max_min_accelerations(self, event="Goal"):
        ...
        self.metrics_results.append(
            self.create_metric(
                ...
                [float(np.mean(linear_acc)), float(np.mean(yaw_acc))],
            )
        )
    ...
    def cumulative_heading_average(self, goal_pose, event="Goal"):
        ...
        self.metrics_results.append(
            self.create_metric(
                "Cumulative_heading_average", float(np.mean(delta_thetas))
            )
        )

    def obstacle_clearance(self, event="Goal"):
        """ """
        ...
        self.metrics_results.append(
            self.create_metric(
                "Obstacles_mean_distance", float(np.mean(mean_distances))
            )
        )
        self.metrics_results.append(
            self.create_metric("Obstacles_min_distance", float(np.min(min_distances)))
        )
        self.metrics_results.append(
            self.create_metric(
                "Obstacles_min_mean_distance", float(np.min(mean_distances))
            )
        )
    ...
    def following_heading_metrics(self, event="Goal"):
        ...
        self.metrics_results.append(
            self.create_metric(
                "Following_heading_metrics", [mean, mae, rmse, max_value]
            )
        )
    ...
    def row_crop_path_comparison(
        self, coordinate, true_path=None, plants_row1=None, plants_row2=None
    ):
        """ """
        path = np.array(self.path)

        if true_path is None:
            true_path = self.compute_gt_rowcrop_path(
                coordinate, path, plants_row1, plants_row2
            )
        else:
        ...
        self.metrics_results.append(self.create_metric("row_crop_path_MAE", float(mae)))
        self.metrics_results.append(self.create_metric("row_crop_path_MSE", float(mse)))
