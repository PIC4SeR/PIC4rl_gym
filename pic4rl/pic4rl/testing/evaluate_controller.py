...
class EvaluateNav(Node):
    def __init__(self):
        ...
        self.controller_package = "row_crop_follow"
        self.controller_name = "segmentation"
        ...
        self.nav_metrics = Navigation_Metrics(train_params, self.logdir)

        self.get_logger().info("Evaluate Navigation: Starting process")
        self.get_logger().info(
            "Navigation metrics collected at: " + str(self.params_update_freq) + " Hz"
        )
    ...
    def _step(self, reset_step=False):
        ...
        self.nav_metrics.get_metrics_data(
            lidar_measurements, self.episode_step, done=done
        )
    ...
    def process_odom(self, odom):
        goal_distance = math.sqrt(
            (self.goal_pose[0] - odom[0]) ** 2 + (self.goal_pose[1] - odom[1]) ** 2
        )

        path_theta = math.atan2(
            self.goal_pose[1] - odom[1], self.goal_pose[0] - odom[0]
        )
    ...
    def respawn_robot(self, index):
        ...
        self.get_logger().info(
            f"Test Episode {self.episode+1}, robot pose [x,y,yaw]: {self.start_pose}"
        )
        logging.info(
            f"Test Episode {self.episode+1}, robot pose [x,y,yaw]: {self.start_pose}"
        )
    ...
    def respawn_goal(self, index):
        ...
        self.get_logger().info(
            f"Test Episode {self.episode+1}, goal pose [x, y]: {self.goal_pose}"
        )
        logging.info(
            f"Test Episode {self.episode+1}, goal pose [x, y]: {self.goal_pose}"
        )
    ...
    def create_logdir(self, logdir):
        ...
        logging.basicConfig(
            filename=os.path.join(self.logdir, "screen_logger.log"), level=logging.INFO
        )

    def evaluate_controller(self, start_controller=True):
        """"""
        self.unpause()
        ...
        if start_controller:
            subprocess.run(
                "killall -9 " + self.controller_name,
                shell=True,
                stdout=subprocess.DEVNULL,
            )
...
def main(args=None):
    ...
    try:
        eval_node.evaluate_controller(start_controller=True)
    except Exception as e:
        eval_node.get_logger().error(
            "Error in starting nav evaluation node: {}".format(e)
        )
