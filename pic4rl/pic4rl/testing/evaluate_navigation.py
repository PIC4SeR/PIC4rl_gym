...
class EvaluateNav(Node):
    def __init__(self):
        ...
        self.get_cli_costmap = self.create_client(
        ...
        if self.navigator.isTaskComplete():
            ...
            if result == TaskResult.FAILED or result == TaskResult.CANCELED:
                ...
                if self.n_navigation_end == 20:
                    self.get_logger().info(
                        "Navigation aborted more than 20 times navigation in pause till next episode"
                    )
        ...
        while rclpy.ok():
            ...
            if future.done():
                try:
                    get_response = future.result()
                    self.get_logger().info(
                        "Result %s " % (get_response.values[0].double_value)
                    )
                except Exception as e:
                    self.get_logger().info("Service call failed %r" % (e,))
                break
        ...
        for n in range(self.n_experiments):
            for episode in range(len(self.goals)):
                ...
                self.nav_metrics.calc_metrics(
                    self.episode, self.start_pose, self.goal_pose
                )
