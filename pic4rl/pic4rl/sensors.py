...
class ImuSensor():
    ...
    def process_data(self, data):
        _,_,yaw = env_utils.euler_from_quaternion(data.orientation)
...
class OdomSensor():
    ...
    def process_data(self, data, vel=False):
        ...
        zr  = env_utils.quat_to_euler(zqr, wqr)
        ...
        if vel:
            ...
            _,_,yaw = env_utils.euler_from_quaternion(data.pose.pose.orientation)
            return [pos_x, pos_y, yaw], [0., 0.]
...
class Sensors():
    def __init__(self, node, testing=False):
        ...
        self.params = self.get_param(testing)
    ...
    def get_param(self, testing):
        ...
        configParams = env_utils.get_ros_params(
            pkg, 'config/params.yaml', 'params')

        return configParams

    def activate_sensors(self):
        if self.params.imu_enabled == "true":
        ...
        if self.params.camera_enabled == "true":
            ...
            self.depth_process = DepthCamera(
                self.params.depth_width, 
                self.params.depth_height, 
                self.params.dist_cutoff,
                self.params.show_depth)
            ...
            #     self.rgb_camera_cb,
            #     1)
            # self.rgb_process = RGBCamera(
            #     self.params.rgb_width, 
            #     self.params.rgb_height, 
            #     self.params.show_rgb)
            # self.sensor_msg['rgb'] = 'None'
            
        if self.params.lidar_enabled=="true":
            self.node.get_logger().debug('Laser scan subscription done')
            self.laser_sub = self.node.create_subscription(
            LaserScan,
            self.params.laser_topic, 
            self.laser_scan_cb,
            1)
            self.laser_process = LaserScanSensor(
                self.params.lidar_distance, 
                self.params.lidar_points, 
                self.params.robot_type,
                self.params.robot_radius,
                self.params.robot_size,
                self.params.coll_tol
                )
