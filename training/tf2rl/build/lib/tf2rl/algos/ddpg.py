...
from tf2rl.networks.NeuralNetworks import Actor, Critic
...
class DDPG(OffPolicyAgent):
    ...
    def get_action(self, state, test=False, tensor=False):
        ...
        if np.random.rand() <= self.epsilon:
            v_rand = np.random.random()*self.actor.max_action[0]
            w_rand = (np.random.random()*2-1)*self.actor.max_action[1]
