...
class Trainer:
    ...
    def __call__(self):
        ...
        while total_steps < self._max_steps:
            if exploration_ep:
                ...
                action = np.array([0.468, 0.001])
                noise = np.random.normal(loc=0.0, scale=0.05, size=action.shape)
                action += noise
                action = np.minimum(action, 5.0)

            elif not exploration_ep and (total_steps < self._policy.n_warmup):
    ...
    def evaluate_policy(self, total_steps, n_episode):
        ...
        for i in range(self._test_episodes):
            episode_return = 0.
            frames = []
            obs, _ = self._test_env.reset(n_episode, total_steps, evaluate=True)
            avg_test_steps += 1
