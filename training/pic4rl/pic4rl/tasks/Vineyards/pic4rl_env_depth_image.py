...
class Pic4rlEnvironment(Node):
	...
	def process_laserscan(self,laserscan_msg):
		...
		self.min_obstacle_distance = np.amin(scan_range)
		#self.max_obstacle_distance = max(scan_range)
	...
	def process_depth_image(self,frame):
		...
		noise = np.random.normal(loc=0.0, scale=1.0, size=depth_frame.shape)*depth_frame/max_depth
		print(np.amax(noise))
		depth_frame = depth_frame + noise
		print(np.amax(depth_frame))
		print(np.amin(depth_frame))
		if self.show_img:
