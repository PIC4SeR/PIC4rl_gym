...
class Pic4rlEnvironmentCamera(Node):
    def __init__(self):
        ...
        parameters=[
            ('data_path', '/root/gym_ws/src/PIC4rl_gym/training/pic4rl/goals_and_poses/vineyard.json'),
    ...
    def _step(self, twist=Twist(), reset_step = False):
        ...
        lidar_measurements, depth_image, goal_distance, goal_angle, pos_x, pos_y, yaw = self.process_sensor_data(sensor_data)
        # Check events (failure,timeout, success)
        done, event = self.check_events(lidar_measurements, goal_distance, yaw, self.episode_step)

        if not reset_step:
            # Get reward
            reward = self.get_reward(twist, goal_distance, goal_angle, pos_x, pos_y, yaw, done, event)
            observation = self.get_observation(twist,depth_image, goal_distance, goal_angle, pos_x, pos_y, yaw)
        else:
            reward = None
            observation = None
    ...
    #############"""
    def respawn_entity(self, index):

        self.goal_x, self.goal_y = self.get_goal(index)
        #Goal initialization
        # Entity 'goal'
    ...
    def respawn_robot(self, index):
        ...
        self.robot_flag = False
        x, y , yaw = self.get_robot_pose(index)
    ...
    def process_sensor_data(self,sensor_data):
        self.get_logger().debug("process sensor data ...")
        ...
        return lidar_measurements, depth_image, goal_distance, goal_angle, pos_x, pos_y, yaw

    def check_events(self, lidar_measurements, goal_distance, yaw, step):

        min_range = self.collision_check
        min_lidar = min(lidar_measurements)
        self.get_logger().debug("Min lidar: {}".format(min_lidar))
        if math.fabs(yaw) > 1.57:
            yaw = math.fabs(yaw) - 1.57
        yaw_limit = math.fabs(yaw)-1.4835  #check yaw is less than 85°
    ...
    def get_observation(self, twist, depth_image, goal_distance, goal_angle, pos_x, pos_y, yaw):
    ...
    def get_reward(self,twist, goal_distance, goal_angle, pos_x, pos_y, yaw, done, event):
        if self.episode < 200:
